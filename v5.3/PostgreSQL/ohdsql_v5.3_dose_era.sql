...
IF OBJECT_ID('@cdmDatabaseSchema.dose_era', 'U') IS NOT NULL DROP TABLE @cdmDatabaseSchema.dose_era;
...
(
	...
	FROM @cdmDatabaseSchema.drug_exposure d
	     JOIN @vocabDatabaseSchema.concept_ancestor ca ON ca.descendant_concept_id = d.drug_concept_id
	     JOIN @vocabDatabaseSchema.concept c ON ca.ancestor_concept_id = c.concept_id
       join @cdmDatabaseSchema.observation_period op on d.person_id = op.person_id
